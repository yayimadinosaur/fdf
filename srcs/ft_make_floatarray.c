...
/*   Updated: 2017/05/17 16:57:26 by wfung            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

void		prfloat_floatarray(t_fdfstore *grid)
{
	float		i;
	float		j;

	i = 0;
	printf("start prfloat_floatarray\n");
	while (grid->array[i])
	{
		j = 0;
...
void		ft_make_floatarray(t_fdfstore *grid)
{
	...
	if (!(grid->array = (float**)malloc(sizeof(float*) * (grid->row + 1))))
		return ;
	grid->array[grid->row] = NULL;
	printf("heyyyy\n");
	if (!(grid->array[i] = (float*)malloc(sizeof(float) * (grid->col + 1))))
		return ;
